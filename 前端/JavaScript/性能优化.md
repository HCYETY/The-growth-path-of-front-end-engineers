...
|[performance.timing.属性](https://developer.mozilla.org/zh-CN/docs/Web/API/PerformanceTiming)|描述|
|:--:|:--:|
|navigationStart|准备加载页面的时间点，相当于在 URL 输入栏回车或者页面按 F5 刷新的时间点。如果没有上一个文档，则返回值等同于fetchStart属性的值|
...
|domainLookupStart、domainLookupEnd|分别表示域名查询开始和结束的时间点。如果直接使用了缓存，则两者的值都等于 fetchStart|
|connectStart、connectEnd|分别表示浏览器与服务器之间的连接开始和建立时的时间点。如果要开始或建立的是持久连接，则返回值等同于fetchStart属性的值|
...
|domContentLoadedEventStart、domContentLoadedEventEnd|分别表示DOMContentLoaded 事件发生和结束的时间点|
